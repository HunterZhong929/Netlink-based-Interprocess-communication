...
static void hello_nl_recv_msg(struct sk_buff *skb)
{
	...
	if(msg[0]== PUB_REG_FLAG){
		...
		//TODO add checking mechanism to make sure the pid is not repeated
		struct linkedList *pub_node;
		pub_node = kmalloc(sizeof(struct linkedList), GFP_KERNEL);
		pub_node->pid=pid;
		struct linkedList *ptr = NULL;
		list_add_tail(&pub_node->list, &pub_list);

		printk("new entry in publist updated:\n");
		list_for_each_entry(ptr, &pub_list, list) {
			printk(" Pub List - PID: %d", ptr->pid);
			}
	}
	//printk("msg = %s", msg);
	...
	if(msg_type == 3){
		printk("normal message sending out the message\n");
	...
	// 	//char* reg_msg = msg_type == 1 ? "Succesfully registered as publisher\n" : "Succesfully registered as subscriber\n";
...
{	
	...
	list_for_each_entry_safe(ptr, next, &sub_list, list) {
        list_del(&ptr->list);
        kfree(ptr);
    }

    
    list_for_each_entry_safe(ptr, next, &pub_list, list) {
        list_del(&ptr->list);
        kfree(ptr);
    }

    printk(KERN_INFO "exiting pubsub module\n");
